...
class SCNetWaveMFFMTransMod(nn.Module):
    def __init__(self, dilA, dilB, dropA, dropB, enc_layers, num_heads):
        self.dropA = dropA
        super(SCNetWaveMFFMTransMod, self).__init__()
    ...
    def forward(self, x):
        ...
        gkp = torch.from_numpy(kurtosis(x.cpu().numpy(), axis=1, keepdims=True)).to(x.device)
